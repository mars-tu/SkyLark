...
# Calling the rosservices
    ...
    def auto_set_mode(self):
        ...
        try:
            # Creating a proxy service for the rosservice named /mavros/set_mode for arming the drone
            setmode_ser = rospy.ServiceProxy(
                'mavros/set_mode', mavros_msgs.srv.SetMode)
            setmode_ser(0, 'AUTO.MISSION')
        except rospy.ServiceException as e:
    ...
    def wpPush(self,index,wps):
        ...
        try:
            # Creating a proxy service for the rosservice named /mavros/mission/push for arming the drone
            pushService = rospy.ServiceProxy(
                'mavros/mission/push', mavros_msgs.srv.WaypointPush)
            pushService(index, wps)
        except rospy.ServiceException as e:
            print("Service arming call failed: %s" % e)
...
def main():
    ...
    w = wayp0.setWaypoints(3,22,True,True,0.0,0.0,0.0,float('nan'),19.134641,72.911706,4)
    wps.append(w)

    w = wayp1.setWaypoints(3,16,False,True,0.0,0.0,0.0,float('nan'),19.134617,72.91188, 5)
    wps.append(w)

    w = wayp2.setWaypoints(3,21,False,True,0.0,0.0,0.0,float('nan'),19.134580,72.911000, 7)
    wps.append(w)
